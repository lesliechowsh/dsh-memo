// Memo retrieval-variant experiment #2 — LongMemEval-S.
// Round 1 showed: length-weighted terms (V2) beats baseline for free; terms +
// bigram pairs with pair weight 2 (V3) tops hit@1/MRR and lifts the weak
// types. This round combines the two and re-baselines in ONE run:
//
//   V0 baseline — product: term counts.
//   V2 — term length weights (sum of token lengths).
//   V3 — term count + bigram count * 2.
//   V5 — term length weights + bigram weight 2 * pair length.
//   V6 — term length weights + bigram weight 1 * pair length.
"use strict";
...
// Merge scoring configs: each returns the ranked session list (top REQUEST_LIMIT).
const VARIANTS = {
  V0: (tokens, pairs, termTop, pairTop) => {
    const counts = new Map(), repTimes = new Map();
    for (const t of tokens) for (const c of termTop.get(t)) {
      if (counts.has(c.id)) counts.set(c.id, counts.get(c.id) + 1);
      else { counts.set(c.id, 1); repTimes.set(c.id, c.time); }
    }
    return [...counts.keys()].sort((a, b) => (counts.get(b) - counts.get(a)) || (repTimes.get(b) - repTimes.get(a))).slice(0, REQUEST_LIMIT);
  },
  V2: (tokens, pairs, termTop, pairTop) => {
    const scores = new Map(), repTimes = new Map();
    for (const t of tokens) for (const c of termTop.get(t)) {
      if (scores.has(c.id)) scores.set(c.id, scores.get(c.id) + t.length);
      else { scores.set(c.id, t.length); repTimes.set(c.id, c.time); }
    }
    return [...scores.keys()].sort((a, b) => (scores.get(b) - scores.get(a)) || (repTimes.get(b) - repTimes.get(a))).slice(0, REQUEST_LIMIT);
  },
  V3: (tokens, pairs, termTop, pairTop) => {
    const scores = new Map(), repTimes = new Map();
    for (const t of tokens) for (const c of termTop.get(t)) {
      if (scores.has(c.id)) scores.set(c.id, scores.get(c.id) + 1);
      else { scores.set(c.id, 1); repTimes.set(c.id, c.time); }
    }
    for (const p of pairs) for (const c of pairTop.get(p)) {
      if (scores.has(c.id)) scores.set(c.id, scores.get(c.id) + 2);
      else { scores.set(c.id, 2); repTimes.set(c.id, c.time); }
    }
    return [...scores.keys()].sort((a, b) => (scores.get(b) - scores.get(a)) || (repTimes.get(b) - repTimes.get(a))).slice(0, REQUEST_LIMIT);
  },
  V5: (tokens, pairs, termTop, pairTop) => {
    const scores = new Map(), repTimes = new Map();
    for (const t of tokens) for (const c of termTop.get(t)) {
      if (scores.has(c.id)) scores.set(c.id, scores.get(c.id) + t.length);
      else { scores.set(c.id, t.length); repTimes.set(c.id, c.time); }
  ...
  V6: (tokens, pairs, termTop, pairTop) => {
    const scores = new Map(), repTimes = new Map();
    for (const t of tokens) for (const c of termTop.get(t)) {
      if (scores.has(c.id)) scores.set(c.id, scores.get(c.id) + t.length);
      else { scores.set(c.id, t.length); repTimes.set(c.id, c.time); }
    ...
    for (const p of pairs) for (const c of pairTop.get(p)) {
      if (scores.has(c.id)) scores.set(c.id, scores.get(c.id) + p.length);
      else { scores.set(c.id, p.length); repTimes.set(c.id, c.time); }
    }
    return [...scores.keys()].sort((a, b) => (scores.get(b) - scores.get(a)) || (repTimes.get(b) - repTimes.get(a))).slice(0, REQUEST_LIMIT);
  },
};
...
function main() {
  ...
  console.log(`evaluating ${questions.length} questions x ${Object.keys(VARIANTS).length} variants`);
  ...
  for (const entry of questions) {
    ...
    for (const name of names) {
      const ranked = VARIANTS[name](tokens, pairs, termTop, pairTop);
      ...
      record(name, String(entry.question_type || "unknown"), rank);
    }
  }
