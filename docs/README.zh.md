...
Memo 的差异化不是一句口号，而是每一条声明都可以被核查：

- **每个数字都来自发布产品本身。** [`bench/`](../bench/README.md) 里的评测脚本复刻的是 `memo_search` 实际运行的管线——分页大小、排序、截断——而不是理想化变体。用相同的数据字节重跑就能得到相同数字，运行环境也已记录。
- **被否掉的实验也公开发表。** 变体实验日志展示了死胡同而不只是赢家：等权 bigram 让 hit@1 跌到 5.2%，加宽每词分页不值 2 倍 API 调用。你看得到试过什么、为什么输。
- **错误在公开记录里纠正。** CHANGELOG 记录了自我纠错：会话 id 读错字段（0.3.0）、标题被静默置空（0.3.1）、发现评测脚本过度收集候选后把数字**下调**重测（0.3.1）、算法真正改进后数字**上调**（0.4.0）、再修掉三个真 bug 后又一次**上调**（0.5.0：停用词挤占内容词窗口、中文查询泄漏全部笔记、手工编辑的笔记文件追加时损坏）。没有悄悄改写的历史。
- **好想法先实验，不靠假设。** LongMemEval 论文的时间感知查询扩展被确定性复现并实测：硬时间过滤**有害**（temporal hit@1 75.9% → 69.9%），软加权变体中性——所以一个都没发货，完整实验公开发表在 [`bench/`](../bench/README.md)。
- **范围明说，不暗示。** 会话定位 ≠ 端到端答题准确率；弱项题型点名列出；0.4.0 算法 2 倍后端调用的成本提前披露。
- **没有稻草人基线。** 你不会看到"本产品并不运行的过程"的比较行，也不会看到把第三方自报数字当作参照行。
...
**口径说明：** 评测测量"会话定位"——金标会话是否进入 top-k——而非端到端答题准确率，也不是论文 M 档（500 会话）的 Recall@k 协议。弱项题型（助手复述类、偏好类）仍是前沿：词面检索在那里有结构性天花板，因为证据常常与问句没有共同词。
