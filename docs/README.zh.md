...
DSH 的 session-query FTS 后端每次调用都对全部活跃语料做对账——慢机器上单次调用要几十秒。Memo 的搜索不走这条路：开机时通过官方精确读取接口（快、不对账）把语料读一遍，之后从内存索引回答。两个刻意的设计后果：

- 索引只收对话记录（用户消息、assistant 完整回复、压缩摘要、会话标题）——这是权威对话内容（已从源头验证：占原始体积 ~93% 的流式碎片和工具机器事件都被折叠或被索引事件取代）。开机建索引期间的搜索返回部分结果并带 `indexing` 进度字段。
- 新鲜度是分钟级：新会话出现在会话列表后即被索引；已增长的老会话在下次重启时重读。笔记搜索不受影响、永远最新。
